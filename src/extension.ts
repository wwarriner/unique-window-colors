...
export class SettingsFileDeleter {
  constructor(private workspaceRoot: string, private colors: ColorsInterface) {}
  ...
  public dispose() {
    ...
    const deleteSettingsFileUponExit = JSON.parse(
      JSON.stringify(
        workspace
          .getConfiguration("windowColors")
          .get<string>("🌈 DeleteSettingsFileUponExit")
      )
    );
    ...
    if (deleteSettingsFileUponExit) {
      ...
      Object.keys(settingsFileJson).length === 1 &&
      Object.keys(cc).length === 3
    ) {
      const aColorWasModified =
        (cc["activityBar.background"] !== this.colors.sideBarColor_dark.hex() &&
          cc["activityBar.background"] !==
            this.colors.sideBarColor_light.hex()) ||
        (cc["titleBar.activeBackground"] !==
          this.colors.titleBarColor_dark.hex() &&
          cc["titleBar.activeBackground"] !==
            this.colors.titleBarColor_light.hex()) ||
        (cc["titleBar.activeForeground"] !==
          this.colors.titleBarTextColor_dark.hex() &&
          cc["titleBar.activeForeground"] !==
            this.colors.titleBarTextColor_light.hex());
...
export function activate(context: ExtensionContext) {
  ...
  const extensionTheme = workspace
    .getConfiguration("windowColors")
    .get<string>("🌈 Theme");
  let baseColor = workspace
    .getConfiguration("windowColors")
    .get<string>("🌈 BaseColor");
  if (baseColor) {
  ...
  if (baseColor) {
    ...
    if (titleBarColor.luminosity() > 0.5) {
      //a light color https://www.npmjs.com/package/color#luminosity
  ...
  if (
    ...
    (cc["activityBar.background"] ||
      cc["titleBar.activeBackground"] ||
      cc["titleBar.activeForeground"] ||
      cc["titleBar.inactiveBackground"])
  ) {
    //don't overwrite
  ...
  if (doUpdateColors || doRemoveColors) {
    const newColors = {
      ...
      "titleBar.inactiveBackground": doRemoveColors
        ? undefined
        : titleBarColor.hex()
  ...
  const settingsFileDeleter = new SettingsFileDeleter(workspaceRoot, {
    sideBarColor_dark,
